/**
 * Backfill multi-univers — ÉTAPE 2b, phase A. Tables de score.
 *
 *   npx tsx scripts/backfill-scores.ts
 *
 * À lancer APRÈS le `db:push` + `db:generate` de la phase A 2b (colonne
 * NULLABLE `universeId` sur les 7 tables de score). Idempotent.
 *
 * Tague toutes les lignes des 7 tables de score → univers JJK.
...
async function main() {
  ...
  // Contrôle : plus aucune ligne à NULL sur les 7 tables (bloquant phase B).
  let remaining = 0;
  for (const table of TABLES) {
  ...
  console.log("✓ Contrôle OK : aucune ligne de score à NULL. Phase B possible.");
}
