/**
 * Seed des ATTRIBUTS de l'univers Bleach — DÉFINITIONS uniquement.
 *
 *   npx tsx scripts/seed-attributes-bleach.ts
 *
 * Écrit en base les définitions de `lib/universes/bleach-attributes.ts` : 8
 * attributs + leurs options. Idempotent (upsert sur les clés uniques) :
 * re-jouable sans effet de bord ni doublon.
 *
 * Copie conforme de `seed-attributes-tg.ts` (mêmes upserts, mêmes contrôles),
 ...
 *   npx tsx scripts/seed-character-attributes.ts --universe bleach --file <json>
 */
...
async function main() {
  ...
  for (const [position, spec] of BLEACH_ATTRIBUTES.entries()) {
    const data = {
      label: spec.label,
      kind: spec.kind,
  ...
  console.log(
    `✓ ${BLEACH_ATTRIBUTES.length} définition(s) seedée(s) — ${total} attribut(s) ` +
  ...
  console.log(
    `ℹ ${filled} valeur(s) d'attribut renseignée(s) pour l'univers ${bleach.slug}.`,
  );
}

main()
  .then(() => console.log("Seed attributs Bleach terminé."))
  .catch((e) => {
    console.error("Seed attributs Bleach échoué :", e);
    process.exitCode = 1;
  })
