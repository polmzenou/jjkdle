/**
 * Seed des ATTRIBUTS de l'univers KNY (DÉFINITIONS uniquement).
 *
 *   npx tsx scripts/seed-attributes-kny.ts
 *
 * Écrit en base les définitions de `lib/universes/kny-attributes.ts` : 8
 * attributs + leurs options. Idempotent (upsert sur les clés uniques) :
 * re-jouable sans effet de bord ni doublon.
 *
 * Copie conforme de `seed-attributes-aot.ts` (mêmes upserts, mêmes contrôles),
 ...
 *   npx tsx scripts/seed-character-attributes.ts --universe kny --file <json>
 */
...
async function main() {
  ...
  for (const [position, spec] of KNY_ATTRIBUTES.entries()) {
    const data = {
      label: spec.label,
      kind: spec.kind,
  ...
  console.log(
    `✓ ${KNY_ATTRIBUTES.length} définition(s) seedée(s) — ${total} attribut(s) ` +
  ...
  console.log(
    `ℹ ${filled} valeur(s) d'attribut renseignée(s) pour l'univers ${kny.slug}.`,
  );
}

main()
  .then(() => console.log("Seed attributs KNY terminé."))
  .catch((e) => {
    console.error("Seed attributs KNY échoué :", e);
    process.exitCode = 1;
  })
