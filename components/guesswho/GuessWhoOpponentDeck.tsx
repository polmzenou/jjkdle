...
interface GuessWhoOpponentDeckProps {
  ...
  /** Titre du panneau (défaut « Deck adverse »). */
  title?: string;
}

/**
 * Miroir en direct d'un plateau : mêmes 25 cartes, grisées au fur et à mesure
 * que le joueur élimine. Lecture seule (aucune interaction).
 */
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col rounded-2xl border border-white/10 bg-void-800/50 p-3">
      <div className="mb-2 flex shrink-0 items-center justify-between gap-2">
        <p className="truncate text-xs font-bold uppercase tracking-[0.2em] text-white/50">
          {title}
        </p>
